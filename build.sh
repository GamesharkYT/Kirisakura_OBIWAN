...
KERNEL_DEFCONFIG=niko/niko_defconfig
Device="ROG 3 BC"
ANYKERNEL3_DIR=$PWD/AnyKernel3/
KERNELDIR=$PWD/
FINAL_KERNEL_ZIP=Niko.zip
...
make -j$(nproc --all) O=out \
                      ...
                      STRIP=llvm-strip
python "$KERNEL_DIR/scripts/ufdt/libufdt/utils/src/mkdtboimg.py" \
...
if [ -f out/arch/arm64/boot/Image.gz ]; then
  ...
  echo "**** Copying Image and dtb ****"
  cp $PWD/out/arch/arm64/boot/Image $ANYKERNEL3_DIR/
  cp $PWD/out/arch/arm64/boot/dtbo.img $ANYKERNEL3_DIR/
  cp $PWD/out/arch/arm64/boot/dtb $ANYKERNEL3_DIR/
